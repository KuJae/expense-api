...
2주차에 메모 앱으로 CRUD를 따라 쳐 봤고, 이번 주에 같은 구조를 처음부터 다시 만들었다.

요청 모델과 응답 모델을 나누는 이유를 알았다. 등록할 때는 `id`와 `created_at`이 없는 게 맞고
(서버가 만든다) 돌려줄 때는 있어야 한다. 하나로 합치면 어느 필드가 필수인지 불분명해진다.

검증은 선언으로 끝났다. `amount`에 -1, `type`에 `"unknown"`, `category`에 빈 문자열을 한 번에
보냈더니 세 건을 각각 이유와 함께 422로 돌려줬다. 내가 쓴 검증 코드는 `Field(gt=0)` 같은 선언
몇 줄이고, `create_transaction` 본문은 실행되지 않았다.

단계 5의 파일 분리는 기능은 그대로고 배치만 바뀐다. 나눈 뒤에는 데이터 모양은 `models.py`,
경로는 `routers/`로 찾을 자리가 정해진다. 옮길 때 `@app.`을 `@router.`로 바꾸는 걸 놓치기 쉬워서
옮기고 나서 파일을 한 번 확인했다.

배포 절차는 2주차와 같고 Start Command만 다르다. 2주차는 `uvicorn main:app`이었는데 이번엔
`app/` 패키지로 옮겼으니 `uvicorn app.main:app`이다. 코드는 고치지 않고 그대로 배포했다.

해결 못 한 것 — 서버를 끄면 데이터가 사라진다. SQLite로 바꾸는 방법이 심화에 있지만 4주차가
정식이라 넘겼다. Render 무료 플랜이 15분마다 잠드는 것도 이번 주 범위에서는 정상으로 두고
넘어갔다.

AI 사용 — Claude Code한테 워크북 단계 1~6을 그대로 따라 만들게 했다. 결과는 직접 호출해서
확인했다. 로컬에서 `/`, `/health`, `/docs`가 뜨는지, 등록이 201로 `id`를 돌려주는지, 없는 id가
404인지, `abc`가 422인지, 삭제가 204이고 같은 id를 다시 지우면 404인지를 하나씩 호출해 상태
코드와 본문을 확인했다(위 ① 기록). 배포 후에는 같은 확인을 인터넷 주소로 한 번 더 돌렸다.
